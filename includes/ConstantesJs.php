<script>
/*para usar los nombres de tabs en los js*/
var tabs = {};

tabs.T1 = '<?php echo T1; ?>';  
tabs.T2 = '<?php echo T2; ?>'; 
tabs.T3 = '<?php echo T3; ?>';
tabs.T4 = '<?php echo T4; ?>';
tabs.T5 = '<?php echo T5; ?>';
tabs.T6 = '<?php echo T6; ?>';
tabs.CRAP = '<?php echo CRAP; ?>';
tabs.CLSF = '<?php echo CLSF; ?>';
tabs.SBCLF = '<?php echo SBCLF; ?>';
tabs.CLSFD = '<?php echo CLSFD; ?>';
tabs.SBCLD = '<?php echo SBCLD; ?>';
tabs.ESPE = '<?php echo ESPE; ?>';
tabs.BANK = '<?php echo BANK; ?>';
tabs.CTACT = '<?php echo CTACT; ?>';
tabs.TDCID = '<?php echo TDCID; ?>';
tabs.PTPG = '<?php echo PTPG; ?>';
tabs.CTPCT = '<?php echo CTPCT; ?>';
tabs.KRGO = '<?php echo KRGO; ?>';
tabs.AARA = '<?php echo AARA; ?>';
tabs.KTRBA = '<?php echo KTRBA; ?>';
tabs.PCNCP = '<?php echo PCNCP; ?>';
tabs.HACT = '<?php echo HACT; ?>';
tabs.DVCL = '<?php echo DVCL; ?>';
tabs.VNFA = '<?php echo VNFA; ?>';
tabs.EMPL = '<?php echo EMPL; ?>';
tabs.ESOG = '<?php echo ESOG; ?>';
tabs.HRIO = '<?php echo HRIO; ?>';
tabs.SISPE = '<?php echo SISPE; ?>';
tabs.TPLL = '<?php echo TPLL; ?>';
tabs.MTBA = '<?php echo MTBA; ?>';
tabs.GNCTR = '<?php echo GNCTR; ?>';